...
int32 SSpineWidget::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyClippingRect, FSlateWindowElementList& OutDrawElements,
	...
	if (widget && widget->skeleton && widget->Atlas) {
		...
		if (widget->atlasNormalBlendMaterials.Num() != widget->Atlas->AtlasTexturePageMap.Num()) {
			...
			for (int i = 0; i < widget->Atlas->GetAtlas()->getPages().Num(); i++) {
				AtlasPage& currPage = widget->Atlas->GetAtlas()->getPages()[i]; // change page pointer error

				UMaterialInstanceDynamic* material = UMaterialInstanceDynamic::Create(widget->NormalBlendMaterial, widget);
				material->SetTextureParameterValue(widget->TextureParameterName, widget->Atlas->AtlasTexturePageMap[currPage.name]);
			...
			for (int i = 0; i < widget->Atlas->GetAtlas()->getPages().Num(); i++) {
				...
				UTexture2D* texture = widget->Atlas->AtlasTexturePageMap[currPage.name];
...
void SSpineWidget::UpdateMesh(int32 LayerId, FSlateWindowElementList& OutDrawElements, const FGeometry& AllottedGeometry, Skeleton* Skeleton) {
	...
	for (int i = 0; i < (int)Skeleton->getSlots().Num(); ++i) {
		...
		if (attachment->getRTTI().isExactly(RegionAttachment::rtti)) {
			...
			attachmentAtlasRegion =  regionAttachment->AttachmentAtlasRegion; //regionAttachment->getRendererObject();
			...
			attachmentAtlasRegion = mesh->AttachmentAtlasRegion;//(AtlasRegion*)mesh->GetRendererObject();
		...
		case BlendMode_Normal:
			if (!widget->pageToNormalBlendMaterial.Contains(AtlasPage)) {
				clipper.clipEnd(*slot);
				continue;
			}
			material = widget->pageToNormalBlendMaterial[AtlasPage];
			break;
		case BlendMode_Additive:
			if (!widget->pageToAdditiveBlendMaterial.Contains(AtlasPage)) {
				clipper.clipEnd(*slot);
				continue;
			}
			material = widget->pageToAdditiveBlendMaterial[AtlasPage];
			break;
		case BlendMode_Multiply:
			if (!widget->pageToMultiplyBlendMaterial.Contains(AtlasPage)) {
				clipper.clipEnd(*slot);
				continue;
			}
			material = widget->pageToMultiplyBlendMaterial[AtlasPage];
			break;
		case BlendMode_Screen:
			if (!widget->pageToScreenBlendMaterial.Contains(AtlasPage)) {
				clipper.clipEnd(*slot);
				continue;
			}
			material = widget->pageToScreenBlendMaterial[AtlasPage];
			break;
		default:
			if (!widget->pageToNormalBlendMaterial.Contains(AtlasPage)) {
				clipper.clipEnd(*slot);
